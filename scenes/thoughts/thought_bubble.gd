...
func say(text: String, anchor: Rect2) -> void:
	if _tween:
		_tween.kill()
	_label.text = text
	...
	var pos := Vector2(anchor.get_center().x - sz.x / 2, anchor.position.y - sz.y - 22)
...
func _draw() -> void:
	...
	var pts := PackedVector2Array([Vector2(_tail_x - 12, y), Vector2(_tail_x + 12, y), Vector2(_tail_x, y + 18)])
